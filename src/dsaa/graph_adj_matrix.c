...
static void gam_free_edge(struct gam *p_gam, uint64_t pos,
...
bool gam_init(struct gam *p_gam, func_data_cmp cmp, uint64_t capacity)
{
	...
	p_gam->idx_array = (uint64_t*)malloc(sizeof(uint64_t) * capacity);
	if (!p_gam->idx_array)
	{
		goto gam_init_err;
	}
	...
	for (uint64_t i = 0; i < capacity * capacity; i++)
	{
		p_gam->edges[i] = NULL;
	}
	...
	func_data_free weight_func_free, void *weight_pool)
{
	...
	if (weight_func_free)
	{
		for (uint64_t i = 0; i < p_gam->capacity * p_gam->capacity; i++)
		{
			if (p_gam->edges[i])
			{
...
bool gam_add_edge(struct gam *p_gam, void *v1_key, void *v2_key, void *weight)
{
	...
	uint64_t row = *(uint64_t*)v1->idx;
	uint64_t col = *(uint64_t*)v2->idx;
	uint64_t pos = row * p_gam->capacity + col;
...
bool gam_find_out_edge(struct gam *p_gam, void *key, struct linked_list *list)
{
	...
	uint64_t pos = 0;
	for (uint64_t i = 0; i < p_gam->capacity; i++)
	{
		pos = *(uint64_t*)vertex->idx * p_gam->capacity + i;
		if (p_gam->edges[pos])
		{
...
bool gam_find_in_edge(struct gam *p_gam, void *key, struct linked_list *list)
{
	...
	for (uint64_t i = 0; i < p_gam->capacity; i++)
	{
		pos = i * p_gam->capacity + *(uint64_t*)vertex->idx ;
		if (p_gam->edges[pos])
		{
			linked_list_append(list, NULL, p_gam->edges[pos]);
		}
	...
	func_data_free weight_func_free, void *weight_pool)
{
	...
	uint64_t pos = *(uint64_t*)v1->idx * p_gam->capacity + *(uint64_t*)v2->idx;
