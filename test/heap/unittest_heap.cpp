...
public:
	void SetUp()
	{
		...
		ret = heap_init(&heap_[0], test_utils_cmp_int, 0);
		ASSERT_TRUE(ret);

		ret = heap_init(&heap_[1], test_utils_cmp_int, TEST_HEAP_LEN);
		ASSERT_TRUE(ret);
	}

	void TearDown()
	{
		heap_destroy(&heap_[0], test_utils_free_int, &test_utils_, test_utils_free_str, &test_utils_);
		heap_destroy(&heap_[1], test_utils_free_int, &test_utils_, test_utils_free_str, &test_utils_);
...
void TestHeapCheckValid(struct heap *p_heap)
{
	for (uint32_t i = 1; i <= p_heap->size; i++)
	{
		struct heap_node *node = &p_heap->nodes[i];
...
TEST_F(HeapFixture, insert)
{
	...
	{
		struct heap *p_heap = &heap_[index];
		...
		for (int i = 0; i < TEST_HEAP_LEN; i++)
		{
			...
			*p = i;
			...
			bool ret = heap_insert(p_heap, p, s);
...
TEST_F(HeapFixture, insert_extract_case1)
{
	for (int index = 0; index < (int)(sizeof(heap_) / sizeof(heap_[0])); index++)
	{
		struct heap *p_heap = &heap_[index];
		...
		for (int i = 0; i < TEST_HEAP_LEN; i++)
		{
			bool ret = heap_extract(p_heap, &node);

			ASSERT_TRUE(ret);

			TestHeapCheckValid(p_heap);

			ASSERT_EQ(*(int*)node.key, TEST_HEAP_LEN - 1 - i);
			char buf[32];
...
TEST_F(HeapFixture, insert_extract_case2)
{
	for (int index = 0; index < (int)(sizeof(heap_) / sizeof(heap_[0])); index++)
	{
		struct heap *p_heap = &heap_[index];
...
TEST_F(HeapFixture, insert_extract_case3)
{
	...
	for (int index = 0; index < (int)(sizeof(heap_) / sizeof(heap_[0])); index++)
	{
		...
		for (int i = 0; i < TEST_HEAP_LEN; i++)
		{
			...
			bool ret = heap_insert(p_heap, &arr[i], s);
...
TEST_F(HeapFixture, insert_extract_find_case)
{
	srand((unsigned int)time(NULL));
	for (int index = 0; index < (int)(sizeof(heap_) / sizeof(heap_[0])); index++)
	{
		...
		for (int i = 0; i < TEST_HEAP_LEN; i++)
		{
			...
			struct heap_node *p_node = heap_find(p_heap, &arr[i]);
		...
		for (int i = 0; i < TEST_HEAP_LEN; i++)
		{
			int tmp = TEST_HEAP_LEN - 1 - i;

			struct heap_node *p_node = heap_find(p_heap, &tmp);
			ASSERT_TRUE(p_node != NULL);
...
TEST_F(HeapFixture, insert_remove_case1)
{
	for (int index = 0; index < (int)(sizeof(heap_) / sizeof(heap_[0])); index++)
	{
		struct heap *p_heap = &heap_[index];
		...
		for (int i = 0; i < TEST_HEAP_LEN; i++)
		{
			struct heap_node *node = heap_find(p_heap, &i);
			ASSERT_TRUE(node != NULL);
			...
			bool ret = heap_remove(p_heap, node, test_utils_free_int, &test_utils_, test_utils_free_str, &test_utils_);
			ASSERT_TRUE(ret);

			node = heap_find(p_heap, &i);
			ASSERT_TRUE(node == NULL);
...
TEST_F(HeapFixture, insert_remove_case2)
{
	for (int index = 0; index < (int)(sizeof(heap_) / sizeof(heap_[0])); index++)
	{
		struct heap *p_heap = &heap_[index];
...
TEST_F(HeapFixture, insert_remove_case3)
{
	for (int index = 0; index < (int)(sizeof(heap_) / sizeof(heap_[0])); index++)
	{
		struct heap *p_heap = &heap_[index];
		...
		for (int i = 0; i < TEST_HEAP_LEN; i++)
		{
			struct heap_node *node = heap_find(p_heap, &arr[i]);
			ASSERT_TRUE(node != NULL);
